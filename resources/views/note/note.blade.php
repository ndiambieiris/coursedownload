        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg p-3">
                <form action="{{ route('note.store')}}" method="post" enctype="multipart/form-data">
                    ...
                    <input type="text"
                        class="form-control" name="name" id="title" aria-describedby="nameID" value="{{ old('name')}}" placeholder="Description">
                        @error('name')
                            <small id="nameID" class="form-text text-danger">{{ $message }}</small>
                        @enderror
                    </div>
                    <div class="mb-3">
                        <label for="note" class="form-label"><strong>Choose Notes</strong></label>
                        <input type="file" class="form-control" name="note" aria-describedby="NoteHelpId" id="NotesHelpId">
                        @error('note')
